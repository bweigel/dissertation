...
```{r, echo=FALSE, results='asis', dev='tikz', fig.width=12, fig.height=5, out.width="\\textwidth"}
...
comp %<>% group_by(fragment, substance, method) %>% #separate(method, into=c("method", "eV"), sep="[.]") %>%  
  ...
  select(fragment, substance, method, mz.int)
...
x <- comp %>% ungroup %>% spread(substance, value = mz.int, fill = NA)
