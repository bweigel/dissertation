...
\ospSetContacts{bweigel@ipb-halle.de\textsuperscript{a}, law@ipb-halle.de\textsuperscript{c}}
...
A newly solved crystal structure of the protein without any bound ligand shows the fully resolved N-terminus, which acts as a lid to close off the active site.
...
Thus, one dataset was solved to completion to obtain a novel \gls{pfomt} structure with no substrate bound and a resolution of \SI{1.95}{\angstrom} (\autoref{tab:xtalstat}).
...
\gls{sah}, \gls{sam} and \gls{sae} were selected to study the influence of the alkyl chain length on binding (\autoref{fig:itc_substances}).
...
Indeed, the value for $\Delta S$ was negative for binding of \gls{sah} and got positive for the binding of \gls{sam} and \gls{sae} (\autoref{tab:ditc}). 
This relationship between the change of entropy and the change of enthalpy has been found for many biological systems and is also called \gls{eec} \citep{Searle1995,Gilli1994,Dunitz1995}.
The stoichiometry for the binding process is given by the parameter $N$ (\autoref{tab:ditc}).
For all the ligands \gls{sah}, \gls{sam} and \gls{sae} this value was found to be about 0.5, which corresponds to one bound molecule ligand per dimer of \pfomt.

Upon titration of caffeic acid to \pfomt small amounts of heat were given off by the system (\autoref{fig:itc.c}). 
When the enzyme was incubated with \gls{sah} prior to addition of caffeic acid this heat increased slightly. The slope of the \gls{itc} profile also got steeper.
However, the data obtained could not be fitted to afford a sensible solution.
When caffeic acid and \cf{Mg^2+} were incubated with \pfomt prior to addition of \gls{sah}, the process of heat production as observed by \gls{itc} also had a steeper slope. 
Nonetheless, the thermodynamic parameters did not differ significantly.
On its own \cf{Mg^2+}, in the form of an \cf{MgCl2} solution, titrated to the enzyme solution did not cause signals during the \gls{itc} experiments.
...
Whereas the binding of \gls{sah} was solely dependent on the large negative enthalpy, the binding of \gls{sae} was almost entirely driven by entropy, since $\Delta H$ was close to 0 (\ref{tab:ditc}).
...
The limited space in the active site, which forces the growing side chain to expel water might also be the reason for the inactivity of \pfomt towards \gls{sae}. 
If the metal ion is blocked from complexing moieties, activation of the substrate hydroxyl would be hindered.

Comparison of the novel \textit{apo}-\pfomt and the published structure suggests a lot of movement along multiple parts of the backbone all promixal to the active site pocket, upon ligand binding (\autoref{fig:d_struc.compare}).
The N-terminus of \pfomt seems to acts as a lid, which is closed in the \textit{apo}-form, but highly flexible and therefore unresolved in the ligand bound form.
...
The laid out findings provide novel hints for regions in the \pfomt structure that can be studied using site-directed mutagenesis approaches in order to work towards a variant that can in fact employ \gls{sae} for transalkylation reactions.
Furthermore variation of these regions might provide enzymes with altered substrate specificities which are of high interest.
...
\framedfigure[label = fig:asite_waters]{%
\includegraphics[width=\textwidth]{usr/graphics/static/asite_compare.png}
}{%
Comparison of the active sites of the \textit{apo}-structure (left, green) and the ligand-bound structure (right, steelblue).
