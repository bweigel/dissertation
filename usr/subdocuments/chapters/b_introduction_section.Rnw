...
\Glsentryfirst{sam} is the universal co-substrate for all \gls{sam} dependent methyl transferases.
...
Nucleophilic transfer of the methyl group of \sam is thermodynamically highly favoured ($\Delta G^0 \approx$ \SI{-70}{\kilo\joule\per\mole} for SAM + homocysteine \textrightarrow~SAH + methionine) and therefore allows the rapid and selective methylation of a range of substrates \cite{Schubert2003}.
\sam is produced by the enzyme \sam synthetase (EC 2.5.1.6) from methionine and \gls{atp} in a two step reaction.
At first \sam is formed and the triphosphate group of \gls{atp} is cleaved off. 
Then, the inorganic triphosphate is hydrolyzed to monophosphate and diphosphate after which the products are released \cite{Takusagawa1996}.
The demethylated \gls{sam} is called \gls{sah}. 
...
Taken together, these reactions leading from and to \sam are commonly called the activated methyl cycle.
...
This reaction proceeds via an S\textsubscript{N}2 mechanism (\autoref{fig:b_methyl_transfer}).%, with \gls{sah} being a very good leaving group.
The methyl group of \gls{sam} is located at a positively charged sulfonium center and is in consequence partially positively charged itself.
This increases the electrophilicity of the methyl group and makes it a strong alkylation agent. 
However, \gls{sam} is still quite stable at physiological conditions compared to other sulfonium species like the trimethylsulfonium ion, which quickly reacts with nucleophiles and is often used for derivatization prior to GC analytics \cite{Butte1983}.
...
An elegant method for the synthesis of chiral acetate made use of glycolytic enzymes to convert glucose via its glycolysis intermediates to [3-\textsuperscript{2}H,\textsuperscript{3}H]-lactate, which is subsequently oxidized by chromiumtrioxide (\autoref{fig:b_chiralacetate}) \cite{Woodard1980}.
...
This experiment also demonstrated that, [\textit{methyl}-\textsuperscript{2}H,\textsuperscript{3}H]-methionine is converted to [\textit{methyl}-\textsuperscript{2}H,\textsuperscript{3}H]-\gls{sam} before the methyl group is transferred by a \gls{mt}.
...
There is no one mechanism that discribes every \gls{mt}.
