...
Non-methylated (\molref*{apigenin}, \molref*{luteolin}) and methylated flavones (\molref*{acacetin} -- \molref*{diosmetin}) show significantly different MS\tup{2} spectra (\autoref{tab:x_flavonfrag}).
Most noteable is the lack of almost any fragmentation of the methylated representatives other than a methyl loss, in \gls{cid} mode (\autoref{fig:chrysoeriol_msspec}).
After the loss of a methyl group a relatively stable radical cation is formed, due to the fact that the whole system is essentially conjugated (\autoref{fig:meflavon_stability}).
%
\begin{figure}[htpb]\centering
  \begin{figureframe}
...
\begin{figure}[htpb]\centering
 ...
 \end{figureframe}
  \caption{ Fragmentation of \molref{chrysoeriol} after \gls{hcd} activation. %
  }
