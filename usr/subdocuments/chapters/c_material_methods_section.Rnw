...
\subsection{Enzymatic production of \glsentrytext{sam} and derivatives}
\label{sec:samprod}
...
The standard assay buffer was 100\,mM Tris/HCl, 2.5\,mM \textsc{L}-glutathion pH\,7.5.
1\,mM \cf{MgCl2}, which was otherwise omitted, was added for reactions using cation dependent \glspl{omt} (e.g. \gls{pfomt}).
Reactions contained \SI{0.5}{\milli\molar} alkyl donor (e.g. (\textit{S},\textit{S})-\glsentrytext{sam}) and \SI{0.4}{\milli\molar} flavonoid or cinnamic acid substrate.
To start the reactions enzyme was added; \SI{20}{\micro\gram}, \gls{pfomt}.
%%%%%%%%%%%%%%%%%%%%%%% Platzhalter
...
When detection of hydrophobic (e.g. flavonoids) and hydrophilic compounds (e.g. \glsentrytext{sam}, \glsentrytext{sah}) was performed simultaneously reactions were stopped by addition of 0.3 volumes 10\,\% (w/v) \gls{tca} in 50\,\% acetonitrile.
...
\subsection{Measurement of activity/pH profiles}
Assays to measure activity over larger pH ranges were set up in 100\,mM \glsentryfirst{mmt}- (pH 4 to 9) or \glsentryfirst{ssg}-buffer (pH 4 to 10) to keep the concentrations of buffer salts constant for each pH.
...
\Gls{omt} enzyme assay extracts (\ref{sec:omtassay}) were analyzed using an acetonitrile gradient starting with 5\,\% acetonitrile (1\,min). 
The acetonitrile content was increased to 100\,\% in 29\,min and was kept at 100\,\% for 5\,min.
