...
Crystallization was sought to answer this question.

At first the already published crystallization procedures were evaluated to start with \citep{Kopycki2008b}. 
...
Crystals began to appear in various wells after a few days (\autoref{fig:d_xtals}) and were observed for each tested protein solution at least once.
The crystal shape varied from very smooth and almost cubic (high ammonium sulfate) over sphreulites and intergrown crystals (\cf{CaCl2}, PEG-4000) to brittle and ragged needles (\cf{LiCl}, PEG-6000).

\framedfigure[label = fig:d_xtals]{%
  \begin{subfigure}{0.18\textwidth}
    \includegraphics[trim=100 0 0 0, clip]{usr/graphics/static/MP000715_C8-3_64d}
  \end{subfigure}
  \begin{subfigure}{0.18\textwidth}
    \includegraphics[trim=100 0 0 0, clip]{usr/graphics/static/MC001414_D4-2_92d}
  \end{subfigure}
  \begin{subfigure}{0.18\textwidth}
    \includegraphics[trim=100 0 0 0, clip]{usr/graphics/static/MC001414_C12-1_3d}
  \end{subfigure}
  \begin{subfigure}{0.18\textwidth}
    \includegraphics[trim=100 0 0 0, clip]{usr/graphics/static/MC001414_D4-1_92d}
  \end{subfigure}
  \begin{subfigure}{0.18\textwidth}
    \includegraphics[trim=100 0 0 0, clip]{usr/graphics/static/MC001414_G10-1_64d}
  \end{subfigure}
}{%
  Some crystal and pseudo-crystal shapes that were observed during the crytsallization screen.
}

Crystals that grew out of high \cf{(NH4)2SO4} conditions were large enough ($\geq$ \SI{50}{\micro\metre} \bweigel{wie groß?}) to be screened for diffraction right away.
From these crystals datasets were obtained and the structures solved.

\ostTableEnv[label = tab:d-xtalstat]{%
...
%  \hfill \\
 ...
 %   \endhead
    ...
    redundancy & \\
    space group & \\
    cell simensions (\gls{a}) & \\
    \textit{a} & \\
    \textit{b} & \\
    \textit{c} & \\ \\
    
    \textit{refinement} & \\
    $R_{\mathrm{work}}$/$R_{\mathrm{free}}$ & \\
    \gls{rmsd} bond lengths (\gls{a}) & \\
    \gls{rmsd} bond angles (°) & \\
    B-values ($\mathrm{\AA^2}$) & \\
    water & \\
    ions & \\ \\
