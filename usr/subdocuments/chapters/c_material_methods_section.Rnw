...
\Gls{somt} was produced only in \glspl{ib}, which were prepared as laid out in \autoref{sec:ibprep}.

\subsection{Preparation of \glsentrytext{ib}}
\label{sec:ibprep}
When insoluble protein is produced in large amounts in \eco it is accumulated in so-called \glsentryfirst{ib}. 
The accumulating \glspl{ib} consist mainly of the overproduced target protein, which is inherently quite pure already.
\Glspl{ib} can be prepared \bweigel{complete}
\citep{Palmer2012}
...
\begin{chemEnv}[0.75]
      \setarrowdefault{-90}
	\schemestart
    	\chemname{\chemfig[]{*6((-H_2N)-(-OMe)=-(-*6(-=-(-NH_2)=(-OMe)-=))=-=)}}{diamine}\arrow{->[*{0}{-2\cf{e^-},2\cf{H^+}}]}
	\chemname{\chemfig[]{*6((-H_2N)-(-OMe)=-(-*6(-=-(-NH_2)=(-OMe)-=))=-=)}}{charge transfer complex}\arrow{<=>}
	\chemname{\chemfig[]{*6((=HN)-(-OMe)=-(-*6(-=-(=NH)-(-OMe)=-))-=--)}}{quinonediimine}
    \schemestop
\end{chemEnv}
