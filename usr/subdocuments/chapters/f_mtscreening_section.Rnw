...
Positive ionization MS\tup{2} spectra of flavanones are mostly characterized by a base peak at \mz{153}, which corresponds to the A-ring of the flavonoid skeleton (\autoref{tab:x_flavonesFrag} and \autoref{fig:flavanone_genfrag}).
...
Losses of water (\SI{18}{\dalton}) and one or two ketene units (\cf{C2H2O}, \SI{42}{\dalton}) are predominant and afford ions of relatively high masses.
...
Thus, the resulting \gls{hcd} spectra generally display smaller \mz{} than the \gls{cid} spectrum.
%
\begin{figure}[htpb]\centering
  \begin{figureframe}
      \includegraphics[scale=1]{usr/graphics/static/flavanone_genfrag}
  \end{figureframe}
...
Only (\frag{1,4}{B}{+}-2H) from eriodictyol \molref{ed} loses a water, as suggested by the peak at \mz{145}.
However, the loss of \cf{CO} is the most prominent decomposition of (\frag{1,4}{B}{+}-2H).
The intensities of the peaks corresponding to the (\frag{1,4}{B}{+}-2H-CO) fragment were as high as \SI{36}{\percent} in \gls{hcd} experiments.
Naringenin \molref{naringenin} seems to sequentially lose two \cf{CO} to afford \mz{91}, which suggests a benzylium or tropylium cation.
...
Another \cf{CO} loss from this fragment is possible for ponciretin \molref{ponciretin} to produce an ion \mz{90}.
...
\begin{figure}[htpb]\centering
  \begin{figureframe}
      \includegraphics[scale=1]{usr/graphics/static/1,3A_fragmentation}
  \end{figureframe}
  \caption{Proposed MS\textsuperscript{2} fragmentation of \frag{1,3}{A}{+} after \gls{hcd} activation.%
  }
  \label{fig:ms2_13A}
\end{figure}
...
\begin{itemize}
  \item ketene (C2H2O) elimination from ring A or C \cite{Kang2007}
