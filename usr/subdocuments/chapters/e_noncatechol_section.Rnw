...
The highest conversion of non-catecholic substrates was observed at high pH and high \cf{Mg^2+} conditions (\autoref{fig:pfomt_conversiopn}).
...
For these substrates the conversion increases by almost 5-fold upon \cf{Mg^2+} addition, which is close to the observed increase of the initial rate of \ifa methylation (\ref{sec:pfomtph}).
...
\section{Structural studies of flavonoids using \glsentryfirst{msms}}
\bweigel{extra kapitel?}

%\section{Consensus or Bioinformatic points-of-view (\glsentrytext{comt})???}
