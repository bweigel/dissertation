...
\begin{lstTabularx}{\linewidth}{>{\small}p{0.25\textwidth}<{\small}X}
  ...
  5$\times$ SDS sample buffer &  10\,\% (w/v) \gls{sds}, 10\,mM \textbeta-mercapto ethanol, 20\,\% glycerol, 0.2\,M Tris/HCl pH\,6.8, 0.05\,\% (w/v) bromophenolblue\\
...
\subsubsection{Preparation of \glsentryfirst{des}}

\Gls{des} were prepared by adding each component in a round-bottom flask with a stirrer and stirring the mixture at \degC{50} with intermittent sonication treatments until a clear solution was obtained.
...
\begin{lstTabularx}{\linewidth}{>{\small}p{0.25\textwidth}<{\small}X}
  ...
  ZYP-5052 & in proportions: 0.928 ZY, 0.05 20$\times$ NPS, 0.02 50$\times$ 5052, 0.002 1\,M \cf{MgSO4}, 0.0002 1000$\times$ trace elements\\
  \addtocounter{table}{-1} 
\end{lstTabularx}
...
\label{sec:ibprep}
Often, when recombinant protein is produced in high levels in \eco it is accumulated in so-called \glsentryfirst{ib}. 
...
The cells were resusupended in 5\,ml/$\mathrm{g_{cells}}$ IB lysis buffer (100\,mM \gls{tris}/HCl, 5\,mM \gls{edta}, 5\,mM \gls{dtt} pH\,7) and 5\,mM benzamidine was added as protease inhibitor.
The solution was homogenized using a tissue grinder homogenizer.
...
The supernatant was discarded and the pellet was resuspended in 5\,ml/$\mathrm{g_{cells}}$ IB wash buffer (100\,mM \gls{tris}/HCl, 5\,mM \gls{edta}, 5\,mM \gls{dtt}, 2\,M urea, 2\,\% (w/v) Triton X-100 pH\,7), followed by thorough homogenization.
The solution was centrifuged (30\,min at \timesg{20000}, \degC{4}), the supernatant discarded and the pellet was washed twice more.
To remove detergent, the pellet was washed twice again. However, for this step the Triton X-100 was omitted from the IB-wash buffer.
The \glspl{ib} were resuspended in extraction buffer (100\,mM \gls{tris}/HCl, 5\,mM \gls{edta}, 5\,mM \gls{dtt}, 8\,M \gls{gua} pH\,7) and stored at \degC{-20} until use.
...
\Gls{pfomt} was crystallized using the following conditions \bweigel{nochma genau guckn}  2\,M \cf{(NH4)2SO4}, 20\,\%glycerol. The protein solution contained 0.25\,mM \gls{sah}, 0.25\,mM \cf{MgCl2}, 0.25\,mM ferulic acid and \SI{7.53}{\mg\per\ml} \gls{pfomt}.

\subsection{Data collection and processing}
Crystallographic data were collected at the beamline of the group of Professor Stubbs (\gls{mlu}, Halle, Germany). 
The rotating anode X-ray source MicroMax007 (Rigaku/MSC, Tokio, Japan) had a maximum power of \SI{0.8}{\kilo\watt} (\SI{40}{\kilo\volt}, \SI{20}{\milli\ampere}) and supplied monochromatic \cf{Cu}-K\textsubscript{\textalpha}-radiation with a wavelength of 1.5418\,\AA.
Diffraction patterns were detected with a Saturn 944+ detector (CCD++, Rigaku/MSC, Tokio, Japan). \\
Indexing and integration of the reflexes via Fourier transformation was accomplished using \emph{XDS} \citep{Kabsch2010,Kabsch1993,Kabsch2010a} or \emph{MOSFLM} \citep{Powell1999}.
\emph{Scala} \citep{Evans2006}, which is integrated in the CCP4-Suite (Collaborative Computational Project, 1994), was used for scaling of the intensities.


\subsection{Structure solution}

\subsection{Model building, refinement and visualization}
 ...
 \end{chemEnv}
  ...
  \begin{subfigure}{0.6\linewidth}\centering
    \begin{chemEnv}[0.75]
      \setarrowdefault{-90}
      \schemestart
    	\Mchemname{\chemfig[][scale=1]{*6((-H_2N)-(-OMe)=-(-*6(-=-(-NH_2)=(-OMe)-=))=-=)}}{\textbf{diamine}}\arrow{->[*{0}{-2\cf{e^-},2\cf{H^+}}]}
    	\chemleft[
	  \Mchemname{\subscheme{
	      \chemfig[]{*6((-H_2N)-(-OMe)=-(-*6(-=-(-NH_2)=(-OMe)-=))=-=)}\arrow{0}[0,0.1]
              \chemfig[]{*6((=HN)-(-OMe)=-(-*6(-=-(=NH)-(-OMe)=-))-=--)}    
          }}{Charge transfer complex}\chemright]
        \arrow{<=>}
