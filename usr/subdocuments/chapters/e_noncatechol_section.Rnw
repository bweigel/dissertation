...
These results display that \gls{doe} combined with \gls{ffd} can be a valueable tool for the identification of main factors during protein refolding.
...
\subsection{Acidity and Nucleophilicity of phenolic hydroxyl-groups}
\subsection{pH-Profiles of \glsentrytext{pfomt}-catalysis}
\subsection{Influence of \cf{Mg^2+} on \glsentrytext{pfomt} activity}
