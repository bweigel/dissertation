...
\begin{tabularx}{\textwidth}{>{\small}p{0.25\textwidth}<{\small}X}
...
\begin{table}[!htb]\centering\small
  \caption{\Glsentryfirst{nades}-mixtures used within this work.}
  \label{tab:des}
  \begin{tabular}{lp{0.3\textwidth}l>{\centering}p{0.3\textwidth}}
    \toprule
    name & composition & mole ratio & mass fraction (w/w) \tabularnewline
    \midrule
    PCH & propane-1,2-diol \newline choline chloride \newline water & 1:1:1 &  0.326 \newline 0.597 \newline 0.077 \tabularnewline
    GCH & \textsc{l}-glucose \newline choline chloride \newline water & 2:5:5 & 0.314 \newline 0.608 \newline 0.078 \tabularnewline
   \bottomrule
  \end{tabular}
\end{table}
...
\begin{tabularx}{\linewidth}{>{\small}p{0.25\textwidth}<{\small}X}
...
\begin{table}[!htb]\centering\small
  \caption{Primers used in this work. Recognition sites for endonucleases are underlined. Positions used for site directed mutagenesis are in lower case font.}
  \label{tab:primers}
  \begin{tabular}{l>{\scriptsize}p{0.65\textwidth}l}
    \toprule
    	name & \small  sequence (5'$\rightarrow$3') & cloning site \\ 
    \midrule
...
\subsection{Software}
All mathematical and statistical computations and graphics were done with the \verb+R+ software (versions 3.1.X, \url{http://cran.r-project.org/}) \citep{RCoreTeam2015}. Visualizations of macromolecules were arranged using the PyMol Molecular Graphics System, version 1.7.0.0 (Schrödinger, New York, USA) or UCSF Chimera version 1.9 (\url{http://www.cgl.ucsf.edu/chimera})\cite{Pettersen2004}.
Phy\-si\-co\-che\-mi\-cal calculations and calculations of different molecular descriptors were performed using Marvin Beans 15.4.13.0 (ChemAxon, Budapest, Hungary) and Molecular Operating Environment 2008.10 (Chemical Computing Group, Montreal, Canada).
...
In vitro site-directed mutagenesis was set-up according to the protocol of the \emph{QuikChange\textsuperscript{\texttrademark} Site-Directed Mutagenesis} kit \cite{Technologies} offered by Agilent Technologies (Santa Clara, USA). 
...
The \textit{SOMT2} gene was amplified from pET28a-SOMT using primers \textit{somt1--somt4} (\autoref{tab:primers}), cloned into vector pICH41308 (level 0) using BpiI and consequently subcloned into the level 1 module pICH75044, alongside 35S promoter and \gls{nos}-terminator, using BsaI (\autoref{fig:cloning}).
The resulting construct was denoted as pBEW107.

\begin{figure}[htb]
  \centering
  \begin{figureframe}
    \includegraphics[width=\textwidth]{usr/graphics/static/cloning_pBEW107}
  \end{figureframe}
  \caption{Golden Gate cloning scheme for \somt}
  \label{fig:cloning}
\end{figure}
...
\subsubsection{Infiltration of \nico for \textit{in vivo} biotransformation using SOMT-2}
Both sides of \nico leaves were infiltrated with different samples (\autoref{fig:c.nico}).
...
\begin{figure}[htb]
  \begin{figureframe}
    \includegraphics[width=0.5\textwidth]{usr/graphics/static/nico2.eps}
  \end{figureframe}
...
\begin{table}\small\centering
  ...
  \begin{tabular}{lc}
	...
	protein/enzyme & $\varepsilon^{\mathrm{\SI{1}{\g\per\l}}}_{280\mathrm{nm}}$ in \si[per-mode=reciprocal]{\ml\per\mg\per\cm}\\ 
     \midrule
     \glsentrytext{pfomt} (reduced) & 0.714 \\
     \glsentrytext{pfomt} Y51K N202W (reduced) & 0.852 \\
     \glsentrytext{somt} (oxidized) & 1.263 \\
     \glsentrytext{somt} (reduced) & 1.247 \\
 %    \glsentrytext{comt} & \\
     \bottomrule
  \end{tabular}
\end{table}
...
Diluted protein samples were concentrated by \gls{tca} precipitation in microcentrifuge tubes.
...
\label{sec:ppprep}
Target proteins may be directed to the periplasmic space by N-terminal signal sequences like \textit{pelB} or \textit{DsbA/C} \citep{Makrides1996}. The periplasma is, other than the cytosol, an oxidizing environment and often used for the production of proteins containing disufide linkages.
...
\label{sec:ibprep}
Often, when recombinant protein is produced at high levels in \eco it is accumulated in so-called \glsentryfirst{ib} \citep{Rudolph1996}. 
...
\label{subsec:doerefolding}
\Gls{doe} and \glsentryfirst{ffd} have been successfully used to optimize the refolding conditions of several proteins \citep{Willis2005,Akbari2010,Benoit2007}.
...
%%   \hfill \\
%%      \toprule
%% 	factor & symbol & \multicolumn{2}{c}{setting (level)} & unit \\ 
%%         & & - & + & \\
%%      \midrule
%%      \endfirsthead
%%      \toprule
...
\begin{table}[htb]\small\centering
  \caption{Factors and their high and low levels (+/-) used in the construction of the \glsentryfirst{ffd}.} 
  \label{tab:ffd_factors}
  \begin{tabular}{lcccc}
    \toprule
    factor & symbol & \multicolumn{2}{c}{setting (level)} & unit \\ 
    ...
    arginine & B & 0 & 0.5 & M \\
    glycerol & C & 0 & 10 & \% (v/v) \\
    divalent cations\textsuperscript{\textdagger} & D & no & yes & - \\
    ionic strength\textsuperscript{\textdaggerdbl} & E & low & high & -  \\
    redox state\textsuperscript{$\star$} & F & reducing & redox-shuffling & - \\
    \textalpha -cyclodextrin & G & 0 & 30 & mM \\
    \glsentrytext{sah} & H & 0 & 0.5 & mM \\
    \bottomrule
    \multicolumn{5}{p{0.95\textwidth}}{\scriptsize \textsuperscript{\textdagger}no: 1\,mM \gls{edta}; yes: 2\,mM \cf{CaCl2}, \cf{MgCl2} \newline \textsuperscript{\textdaggerdbl}low: 10\,mM NaCl, 0.5\,mM KCl; high: 250\,mM NaCl, 10\,mM KCl \newline \textsuperscript{$\star$}reducing: 5\,mM \gls{dtt}; redox-shuffling: 1\,mM \gls{gsh}, 0.2\,mM \gls{gssg}}
  \end{tabular}
\end{table}

\begin{table}[htb]\small\centering
  \caption{Experimental design matrix for the \glsentrytext{ffd}.} 
...
\ul{50} of solubilized \gls{somt} (\SI{1}{\mg\per\ml}) in \gls{ib} solubilization buffer was added to \SI{1}{\milli\litre} of each buffer followed by a short vortex boost for rapid mixing.
...
The performance of each buffer on the refolding of \gls{somt} was examined by comparing the \sdspage results, as well as the amount of soluble protein and the conversion of naringenin over night (see \autoref{sec:omtassay}).
...
Structure visualization and the preparation of figures was performed using PyMOL (Schrödinger, New York, USA) and UCSF Chimera (\url{http://www.cgl.ucsf.edu/chimera})\cite{Pettersen2004}.
...
The reaction was extraced once more with \ul{500} ethyl acetate, 0.2\,\% formic acid and the pooled organic phases were evaporated using a vacuum concentrator (Concentrator 5301; Eppendorf AG, Hamburg, Germany).
...
\subsubsection{Measurement of activity/pH profiles}\label{sec:pHassay}
...
\cf{MgCl2} was either omitted or added at \SI{10}{\milli\molar} to assess the influence of divalent cations.
Assays were stopped as described in \ref{sec:omtassay} and analyzed accordingly.

\subsubsection{Estimation of product concentration and enzymatic activities}
\label{ref:concestimation}

Product concentrations were estimated from \gls{hplc} runs. 
The automatically integrated peaks of \gls{sam} and \gls{sah} provided the \gls{auc}.
From the \gls{auc} of both peaks the concentrations were estimated as follows.
%
\begin{align*}
  \intertext{Under the assumption, that}
  AUC^\mathrm{SAH} + AUC^\mathrm{SAM} &= 1 \sim c_0^\mathrm{SAM}, \\
  \intertext{the fraction and concentration of one (e.g. SAH) can be estimated by}
  x^\mathrm{SAH} &= \frac{AUC^\mathrm{SAH}}{AUC^\mathrm{SAH} + AUC^\mathrm{SAM}} \\
  \intertext{and}
  c^\mathrm{SAH} &= x^\mathrm{SAH} \times  c_0^\mathrm{SAM}.
\end{align*}
%
The amount $n$ is obtained by multiplying the concentration $c$ by the injection volume $V$.
Enzymatic activities (i.e. initial rates) can be calculated from the concentrations by standard procedures.
...
Since the colored complex is specific for caffeic acid and not observed with methylated derivatives (i.e. ferulic and iso-ferulic acid), this can be used as a measure for methylation reactions.
\Gls{omt} assays were prepared as before (\autoref{sec:omtassay}). 
...
\subsection{\Glsentryfirst{lcmsms} measurements}

The positive and negative ion high resolution \gls{esi} and \gls{cid} MSn spectra as well as \gls{hcd} MS/MS spectra were obtained from an Orbitrap Elite mass spectrometer (Thermo Fisher Scientific, Bremen, Germany) equipped with an \gls{hesi} ion source (positive spray voltage \SI{4.5}{\kilo\volt}, negative spray voltage \SI{3.5}{\kilo\volt}, capillary temperature \degC{275}, source heater temperature \degC{250}, \gls{ftms} \gls{rp} \num{30000}). 
Nitrogen was used as sheath and auxiliary gas. 
The MS system was coupled with an \gls{uhplc} system (Dionex UltiMate 3000, Thermo Fisher Scientific), equipped with a RP-C18 column (particle size \SI{1.9}{\micro\metre}, pore size \SI{175}{\angstrom}, 50\,x\,\SI{2.1}{\milli\metre} inner diameter, Hypersil GOLD, Thermo Fisher Scientific, column temperature \degC{30}) and a photodiode array detector (\SIrange{190}{400}{\nano\metre}, Thermofisher Scientific). 
For the \gls{uhplc} a gradient system was used starting from \cf{H2O}:\cf{CH3CN} 95:5 (each containing \SI{0.2}{\percent} formic acid) raised to 0:100 within 10\,min and held at 0:100 for further 3\,min. 
The flow rate was \SI{150}{\micro\litre\per\minute}. 

The mass spectra (buffer gas: helium) were recorded using \glspl{nce} of \SIrange{30}{45}{\percent} and \SIrange{75}{100}{\percent} for \gls{cid} and \gls{hcd} mass spectra respectively (see Appendix).
The instrument was externally calibrated using the Pierce\textsuperscript{\textregistered} LTQ Velos ESI positive ion calibration solution (product number 88323, Thermofisher Scientific, Rockford, IL, 61105 USA) and the Pierce\textsuperscript{\textregistered} LTQ Velos ESI negative ion calibration solution (product number 88324, Thermofisher Scientific, Rockford, IL, 61105 USA) for positive and negative ionization mode respectively.
%The data were evaluated by the Xcalibur software 2.7 (Thermo Fisher Scientific). 
