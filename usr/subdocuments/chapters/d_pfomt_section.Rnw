...
The screened crystals all had similar cell parameters and belonged to the same space group, $P2_12_12_1$, as the previously published structure (pdb: 3C3Y)\citep{Kopycki2008b}.
...
The active site of each monomer was found to be empty except for a sole sulfate ion, which was positioned where the amino- and carboxylate groups of the \gls{sah} residue in the 3C3Y structure (\autoref{fig:d_struct_b}).
...
An overview of the features in the \emph{apo}-\gls{pfomt} structure. \textbf{\subref{fig:d_struct_a}} -- The assymetric unit of \textit{apo}-\gls{pfomt} consists of two homodimers (4 monomers). Individual monomers are rainbow colored from N- (blue) to C-terminus (red). %
 \textbf{\subref{fig:d_struct_b}} -- Comparison of 3C3Y (steelblue) and \textit{apo}-\gls{pfomt} (green). The N-terminus of \textit{apo}-\gls{pfomt} was resolved up to the N-terminus (red) and even the His-tag (red, transparent) was partly resolved.
...
For all the ligands \gls{sah}, \gls{sam} and \gls{sae} this value was found to be about 0.5, which corresponds to one bound molecule ligand per dimer of \pfomt (\autoref{tab:ditc}).
...
\includegraphics[width=0.5\textwidth, trim=300 100 300 100, clip]{usr/graphics/static/d_activesite_resis}
   ...
   The active site of \gls{pfomt} (pdb: 3C3Y). 
...
\begin{figure}[htpb]
  ...
  \end{figureframe}
  \caption{Activities of different \pfomt variants towards caffeic acid methylation.}
  \label{fig:pfomt_DIM}
\end{figure}
