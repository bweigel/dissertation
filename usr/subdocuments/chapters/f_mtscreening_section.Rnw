...
\chapter{Structural studies of flavonoids using \glsentryfirst{msms}}
\label{ch:MSMS}

  
\ospSetTitle{Characterization of flavanoid aglycones and cinnamic acids by liquid chromatography coupled tandem mass spectrometry}
...
In this work a range of flavonoids was studied using \gls{lcms}.
...
In order to still generate fragments in \gls{lcms} \gls{msms} methods such as \gls{cid} or \gls{sid} were developed \cite{Sleno2004}.
...
\begin{figure}[htpb]\centering
  ...
  \end{figureframe}
  \caption{Ion fragment nomenclature of flavonoid aglycones as proposed by Ma \etal, illustrated on naringenin.}
  \label{fig:ion_nomenclature}
\end{figure}

In this work the complementarity of two activation methods, \gls{cid} and \gls{hcd}, for the structural characterization of flavonoids, especially those methylated at the B-ring, in positive ionization mode was evaluated.
%The main focus of this study were flavonoids methylated at the B-ring.

\begin{table}[htpb]\centering\small
  \begin{minipage}{0.7\textwidth}
    \captionof{table}[]{Substrates studied in this work. Three classes of flavonoids were tested: flavanones (\molref{naringenin}-\molref{hed}), flavones (\molref{apigenin}-\molref{chrysoeriol}) and flavonols (\molref{kaempferol}-\molref{myricetin}). The topology of the bond in the C-ring specifying flavanones or flavones/flavonols is denoted with \textbf{s} (single) or \textbf{d} (double), respectively.}
...
%    \caption[Test Caption]{Test}\centering
    \begin{tabularx}{\textwidth}{lXcccccc}
      ...
      \molnum{hesperetin} &  hesperetin & 303 & s & H & OH & OCH\textsubscript{3} & H \\
      \molnum{hed} &  homoeriodictyol & 303 & s & H & OCH\textsubscript{3} & OH & H \\
      \midrule
      \molnum{apigenin} &  apigenin & 271 & d & H & H & OH & H \\
      \molnum{luteolin} &  luteolin & 287 & d & H & OH & OH & H \\
      \molnum{diosmetin} &  diosmetin & 301 & d & H & OH & OCH\textsubscript{3} & H \\
      \molnum{chrysoeriol} &  chrysoeriol & 301 & d & H & OH & OCH\textsubscript{3} & H \\
...
\begin{table}[htpb]\centering\small
  \caption{Product ions formed from the protonated molecular ions [M+H]\textsuperscript{+} of the flavanones \molref{naringenin} -- \molref{hed} in \gls{cid} mode (\SI{45}{\electronvolt}).}
...
%    \multirow{2}{*}{\small ion} & \multicolumn{5}{c}{\small \gls{cid}} & \multicolumn{2}{c}{\small \gls{hcd}} \\
 %   \cmidrule(lr){2-6}\cmidrule(lr){7-8}
  ion  & \small \molref{naringenin} & \small \molref{ponciretin} & \small \molref{ed} & \small \molref{hesperetin} & \small \molref{hed}  \\
    \midrule
   \protect[M+H-\cf{H2O}]\textsuperscript{+} &  1.2 &  1 &  17.8 &  9.8 &  3.8\\
   \protect[M+H-2\cf{H2O}]\textsuperscript{+}  &  &  &  3.7 &  0.8 & \\
   \protect[M+H-\cf{C2H2O}]\textsuperscript{+}  &  3.7 &  2.7 &  3.3 &  2.2 &  2.1  \\
   \protect[M+H-2\cf{C2H2O}]\textsuperscript{+}  &  4.5 &  3.1 & 3 &  2.5 &  1.5  \\
   \protect[M+H-2\cf{C2H2O}-\cf{H2O}]\textsuperscript{+}  &  &  &  4.1 &  &   \\
    \frag{}{AC}{+}  &  3.9 &  5.4 &  20.5 &  27.9 &  30.1   \\
    \frag{1,3}{A}{+}  &  100 &   80.1 &  31 &  22.2 &  57.9  \\
    (\frag{1,4}{B}{+}-2H) &  81.8 &  100 &  100 &  100 &  100  \\
%    (\frag{1,3}{B}{+}-2H) \textsuperscript{\textdagger}  &   2.7 &  4 &  1 &   &   \\
...
\begin{figure}[htpb]\centering
 ...
 %   \begin{subfigure}{\textwidth}
      \includegraphics[scale=1]{usr/graphics/static/hesperetin_HCD}
  %    \caption{}\label{fig:hesp_hcd}
  %  \end{subfigure}%
    
   % \begin{subfigure}{\textwidth}
    %  \includegraphics[scale=1]{usr/graphics/static/CID_hesperetin}
    %  \caption{}\label{fig:hesp_cid}
    %\end{subfigure}%
  \end{figureframe}
  \caption{Major fragmentation pathways of hesperetin upon activation using \gls{hcd} conditions at \SI{75}{\electronvolt}}% or \textbf{\subref{fig:hesp_cid}} \gls{cid} at \SI{45}{\electronvolt}.}
  \label{fig:hcd_frag_hesp}
\end{figure}
...
\section{Fragmentation of flavones}

The \gls{cid}-spectra of the flavones (\molref{apigenin} -- \molref{dimethylluteolin}) are summarized in \autoref{tab:ions_flavones}.
The \mz{} of the molecular ions [M+H]\textsuperscript{+} were observed for the unmethylated compounds apigenin (\molref{apigenin}) and luteolin (\molref{luteolin}).
Conversely the \mz{} corresponding to an ion after methyl loss [M+H-\cf{CH3}]\textsuperscript{+} was the largest observed for the methylated derivatives (\molref{diosmetin} -- \molref{dimethylluteolin}).
The \mz{} of the [M+H-\cf{CH3}]\textsuperscript{+} ion was also the only one observed for the monomethyl derivatives (\molref{diosmetin}, \molref{chrysoeriol}).
In earlier studies it was already shown, that flavones methylated at the B-ring have a low tedency for fragmentation in \gls{cid} experiments \citep{Ma1997}.
In contrast to the monomethylated compounds \molref{diosmetin} and \molref{chrysoeriol}, the dimethylated luteolin-3',4'-dimethylether (\molref{dimethylluteolin}) also showed a loss of methane (\mz{16}) followed by further losses of water (\mz{18}) and \cf{CO} (\mz{28}).

The unmethylated compounds \molref{apigenin} and \molref{luteolin} showed neutral losses of water, \cf{C2H2O}, \cf{C2H2} and carbon monoxide.
The main peak in the spectra of these compounds was at \mz{153}, which corresponds to the \frag{1,3}{A}{+} ion.
...
\begin{figure}[htpb]\centering
  \begin{figureframe}
   \includegraphics[scale=1]{usr/graphics/static/chrysoeriol_HCD}
  \end{figureframe}
  \caption{Major fragmentation pathways of chrysoeriol upon \gls{hcd} at \SI{100}{\electronvolt}}
\end{figure}
...
\begin{table}[htpb]\centering\small
  \caption{Product ions formed from the protonated molecular ions [M+H]\textsuperscript{+} of the flavonols \molref{kaempferol} -- \molref{myricetin} in \gls{cid} mode (\SI{45}{\electronvolt}).}
  \label{tab:ions_flavonols}
  \begin{tabular}{>{\footnotesize}p{0.25\textwidth}cccc}
    \toprule
  ion  & \molref{kaempferol} & \molref{quercetin} & \molref{isorhamnetin} & \molref{myricetin}  \\
    \midrule
...
\begin{table}[htpb]\centering\small
  \caption{Comparison of \gls{cid} (\SI{45}{\electronvolt}) and \gls{hcd}-spectra (\SI{75}{\electronvolt}) of isorhamnetin (\molref{isorhamnetin}) and myricetin-3'-methylether (\molref{myricetin-mme}). Activation via \gls{hcd} affords smaller fragments derived from an upstream C-ring cleavage.}
  \label{tab:cmp_flavonols}
  \begin{tabular}{p{0.25\textwidth}cccc}
    \toprule
    \multirow{2}{*}{\small ion} & \multicolumn{2}{c}{\small \gls{cid}} & \multicolumn{2}{c}{\small \gls{hcd}} \\
    \cmidrule(lr){2-3}\cmidrule(lr){4-5}
    & \small \molref{isorhamnetin} & \molref{myricetin-mme} & \molref{isorhamnetin} & \molref{myricetin-mme} \\
    \midrule
...
%\section{Fragmentation of cinnamic acids}
...
\item \frag{1,3}{A}{+} is usually most abundant and most readily formed \cite{Cuyckens2004} \textrightarrow originates from acetate–malonate pathway, therefore m/z 153; when 4-keto is missing e.g. flavanes flavanols m/z 139
...
Annegret Laub and Jürgen Schmidt conducted \gls{lcms} measurement runs.
