...
\subsubsection{Estimation of product concentration}
...
\begin{align*}
  ...
  AUC^\mathrm{SAH} + AUC^\mathrm{SAM} &= 1 \sim c_0^\mathrm{SAM},x \\
...
Enzymatic activities can be calculated from the concentrations by standard procedures.
