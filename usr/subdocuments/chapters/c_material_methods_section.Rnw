...
Percentages refer to weight per volume (w/v) percentages unless otherwise specified.

\begin{lstTabularx}{\linewidth}{>{\small}p{0.25\textwidth}<{\small}X}
  lysis buffer & 50\,mM Tris/HCl, 500\,mM NaCl, 10\,\% (v/v) glycerol, 2.5\,mM imidazole, 0.2\,\% Tween-20 pH 7\\
  binding buffer & 50\,mM Tris/HCl, 500\,mM NaCl, 10\,\% (v/v) glycerol, 2.5\,mM imidazole pH 7\\
  elution buffer & 50\,mM Tris/HCl, 500\,mM NaCl, 10\,\% (v/v) glycerol, 250\,mM imidazole pH 7 \\
  50$\times$5052 & 25\,\% (v/v) glycerol, 2.5\,\% glucose, 10\,\% \textalpha-lactose \\
  20$\times$NPS & 1\,M \cf{Na2HPO4}, 1\,M \cf{KH2PO4}, 0.5\,M \cf{(NH4)2SO4} \\
...
\begin{lstTabularx}{\linewidth}{>{\small}p{0.25\textwidth}<{\small}X}
  ...
  TB-medium & \SI{12}{\g\per\l} tryptone, \SI{24}{\g\per\l} yeaxst extract, 0.4\,\% (v/v) glycerol, 72\,mM \cf{K2HPO4}, 17\,mM \cf{KH2PO4}\\
  ZYP-5052 & 0.928\,\glsentryfirst{v} 20$\times$NPS, 0.02\,V 50$\times$5052, 0.002\,V 1\,M \cf{MgSO4}, 0.0002\,V 1000$\times$ trace elements\\
\end{lstTabularx}
...
Synthetic genes or gene fragments were obtained from GeneArt (Life Technologies, Darmstadt, Germany) or Eurofins Genomics (Ebersberg, Germany).
...
\subsection{Software}
All mathematical and statistical computations and graphics were done with the \verb+R+ software (versions 3.1.X, \url{http://cran.r-project.org/}). Visualizations of macromolecules were arranged using the PyMol Molecular Graphics System, version 1.7.0.0 (Schrödinger, New York, USA).\\
...
Plasmid DNA was isolated using the QIAprep Spin Miniprep Kit (QIAGEN, Hilden, Germany) according to the manufacturer's instructions. \\
In vitro site-directed mutatgenesis was set-up according to the protocol of the \emph{QuikChange Site-Directed Mutagenesis} kit \cite{Technologies} offered by Agilent Technologies (Santa Clara, USA). \\
...
Stock solutions of antibiotics, IPTG or sugars were prepared according to the pET System Manual by Novagen \citep{Novagen2011}, unless otherwise noted.
...
The concentrations of antiobiotics used was as follows:
...
The protein subfractionation procedure described herein was adapted from the protocol described in the pET Manual \citep{Novagen2011}. 
...
The \emph{PageRuler Prestained Protein Ladder} (Life Technologies GmbH, Darmstadt, Germany) was used as a \gls{mw} marker and run alongside every analysis as a reference. 
...
The main culture (\gls{zyp} containing \ugperml{150} kanamycin) was inoculated with the starter culture such that \odx{600} was 0.05.
...
The pellet was resuspended in 50\,mM Tris/HCl, 500\,mM NaCl, 2.5\,mM imidazole, 10\,\% glycerol pH\,7 using a volume of $\approx$\SI{10}{\milli\litre\per\gram} of cell pellet.
...
Consequently, the His-tagged \gls{pfomt} was purified by \gls{aic}.
...
\subsection{Purification of His-tagged proteins using \glsentryfirst{aic}}
\label{sec:aic}
N- or C-terminal oligo-histidine tags (His-tags) are a common tool to ease purification of recombinantly produced proteins.
The free electron pairs of the imidazol nitrogens of the histidines can complex divalent cations such as \cf{Mg^2+} or \cf{Ni^2+}, which are usually immobilized on a matrix of \gls{nta}-derivatives.
...
First the column was equilibrated with 5\,\gls{cv} of binding buffer (50\,mM Tris/HCl, 500\,mM NaCl, 10\,\% (v/v) glycerol, 2.5\,mM imidazole pH 7).
...
Unspecifically bound proteins were washed away by increasing the amount of elution buffer (50\,mM Tris/HCl, 500\,mM NaCl, 10\,\% (v/v) glycerol, 250\,mM imidazole pH 7) to 10\,\% (constant for 3 to 5\,\gls{cv}). 
Highly enriched and purified target protein was eluted with 6 to 10\,\gls{cv} of 100\,\% elution buffer. 


%%%% analytical methods
\section{Analytics}
\subsection{In vitro determination of glucose}
The glucose concentration in clarified, aqueous samples was determined by a modified version of the glucose assay kit procedure provided by Sigma-Aldrich \citep{Sigma-Aldrich}. \Gls{god} oxidizes D-glucose to gluconic acid, whereby hydrogen peroxide is produced. The hydrogen peroxide can be detected and quantified by \gls{hrp}, which reduces the produced \cf{H2O2} and thereby oxidizes its chromogenic substrate \textit{o}-dianisidine. The oxidized form of \textit{o}-dianisidine can then be measured photospectrometrically \citep{Claiborne1979}.\\
...
\ul{50} 6\,M sulfuric acid was added to stop the reaction and achieve maximum color development (full oxidation of any \textit{o}-dianisidine charge transfer complexes). \\
The developed pink color was measured at 540\,nm in a \gls{mtp}-reader.
A calibration curve of a standard D-glucose solutions (0 to 100\,\si{\micro\gram\per\milli\litre}), that was always part of the experiments, was used to quantify the sample measurements.
...
\framedfigure[label = fig:godassay]{
  \begin{subfigure}{0.4\linewidth}\centering
    \begin{tikzpicture}[node distance=2.5cm]\tiny
     ...
     \node (0,0) (a) {D-Glc + \newline \cf{O2} + \cf{H2O}};
     \node [right of=a] (b) {gluconic acid + \cf{H2O2}};
     \draw[->] (a) -- node[midway, above] {GOD} (b);
		
     \begin{scope}[nodes = {shape=rectangle, draw=none, below=1cm}]
       \node 		at (a) (c) {\cf{H2O2} + \textit{o}-dianisidine};
       \node        at (b) (d) {CTC};
     \end{scope}
     \draw[->] (c) -- node[midway,above] {HRP} (d);
    
     \begin{scope}[->, nodes = {shape=rectangle, draw=none, below = 2cm}]
       \node 		at (a) (e) {CTC};
       \node        at (b) (f) {ox. dianisidine};      
     \end{scope}
     \draw[->] (e) -- node[midway, above] {\cf{H2SO4}} (f);
  ...
  \begin{subfigure}{0.4\linewidth}\centering
    ...
    \setarrowdefault{-90}
	\schemestart
    	\chemname{\chemfig[][scale=0.5]{*6((-H_2N)-(-OMe)=-(-*6(-=-(-NH_2)=(-OMe)-=))=-=)}}{diamine}\arrow{->[*{0}{-2\cf{e^-},2\cf{H^+}}]}
	\chemname{\chemfig[][scale=0.5]{*6((-H_2N)-(-OMe)=-(-*6(-=-(-NH_2)=(-OMe)-=))=-=)}}{charge transfer complex}\arrow{<=>}
	\chemname{\chemfig[][scale=0.5]{*6((=HN)-(-OMe)=-(-*6(-=-(=NH)-(-OMe)=-))-=--)}}{quinonediimine}
    \schemestop
      \end{subfigure}
  
}{GOD assay}

\subsection{\glsentrytext{hplc} measurements}
