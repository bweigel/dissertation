...
The $K_D$ values of \gls{sah}, \gls{sam} and \gls{sae} are in the low micromolar range, all around \SI{2}{\micro\molar}.
...
Thus, the entropic influence must get larger with increasing chain length in order for (\ref{eq:gibbs1}) and (\ref{eq:gibbs2}) to still hold true (\autoref{tab:d-itc}).
Indeed, the value for $\Delta S$ was negative for binding of \gls{sah} and got positive for the binding of \gls{sam} and \gls{sae}.
