...
%\phantomsection
\label{ch:MSMS}
%\addcontentsline{toc}{chapter}{Tandem mass-spectrometry studies of flavonoids}
...
\ospSetContacts{bweigel@ipb-halle.de\textsuperscript{a}, alaub@ipb-halle.de\textsuperscript{b}, jschmidt@ipb-halle.de\textsuperscript{c}, wessjohann@ipb-halle.de\textsuperscript{d}}
...
Flavonoids comprise a huge chemical space, with thousands of theoretical structures \cite{Williams2004}.
...
An specific array of different flavonoids (\autoref{tab:LCMSsubstrates}) was studied, to get a holistic impression of the fragmentations of these compounds.
...
%      \includegraphics[width=\textwidth]{usr/graphics/static/flavanone_genfrag}
       \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-2_flavanone_genfrag}
  \end{figureframe}
...
%      \includegraphics[width=\textwidth]{usr/graphics/static/13A_fragmentation}
    \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-3_1,3A_fragmentation}
  \end{figureframe}
...
\begin{figure}[htpb]\centering
  ...
  \caption{Comparison of \gls{cid} and \gls{hcd} MS\tup{2} spectra of eriodictyol \molref{ed}. 
    ...
    Four different prominent peaks are annotated in each spectrum. \textbf{D} -- The shift to smaller masses in \gls{hcd} spectra and with increasing \gls{nce} is illustrated by the boxplot of the distribution of peaks with relative intensities above \SI{1}{\percent}. Labels a to c correspond to spectra A to C, respectively.
...
%      \includegraphics[width=\textwidth]{usr/graphics/static/deme_flavone_genfrag}
      \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-4a_deme_flavone_genfrag}
...
%      \includegraphics[width=\textwidth]{usr/graphics/static/me_flavone_genfrag}
      \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-4b_me_flavone_genfrag}
...
%      \includegraphics[scale=1]{usr/graphics/static/chrysoeriol_fragmentation}
    \includegraphics[scale=1]{usr/graphics/static/scheme_1-6_chrysoeriol_fragmentation}
 \end{figureframe}
...
Whereas in \gls{cid}, methylated flavones hardly showed any fragmentation beyond a methyl loss, the methylated flavonols kaempferide \molref{kaempferide} and isorhamnetin \molref{isorhamnetin} exhibited the same losses as their non-methylated couterparts, albeit at much lower levels (\autoref{tab:x_flavonolfrag}, \autoref{sche:flavonole_genfrag} and \ref{fig:isorhamnetin_msspec}).
...
Many high intensity peaks presented in the MS\tup{2} spectra of flavonols and the base peaks changed between compounds.
...
%      \includegraphics[width=\textwidth]{usr/graphics/static/flavonole_genfrag}
      \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-7a_flavonole_genfrag}
      \caption{Fragments characteristic for all studied flavonols.}
    \end{subfigure}
    
    \begin{subfigure}{\textwidth}
%      \includegraphics[width=\textwidth]{usr/graphics/static/me_flavonole_genfrag}
      \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-7b_me_flavonole_genfrag}
      \caption{Characteristic fragments of methylated flavonols.}
    \end{subfigure}
 \end{figureframe}
  \caption{Major fragmentation pathways of flavonols. Unlike flavones, methylated and non-methylated flavonols share common fragmentations, albeit signals corresponding to small molecule losses are typically small for methylated analogues.
...
Other than the flavones, flavonols can loose two sequential \cf{CO} and another \cf{C4H4O2}, confirming previously published data \cite{Ma1997}.
...
The data also clearly show, that neutral losses from the molecular ion are most abundant in \gls{cid} experiments, whereas the shift to smaller masses in \gls{hcd} experiments is obvious (\autoref{tab:x_flavonolfrag}, \autoref{fig:isorhamnetin_msspec}).

The studied flavonols all displayed an MS signal at \mz{153} corresponding to the \frag{1,3}{A}{+} fragment, just as the flavanones and flavones with a 5,7-dihydroxy-substitution of the A-ring did.
...
The data suggests, that these [M+H-\cf{CH3OH}]\tup{+} fragments can loose up to two \cf{CO}, similar to the loss of water and \cf{CO} (\autoref{sche:isorhamnetin_frag} and \ref{sche:flavonol_genfrag}).
%
\begin{scheme}[htpb]\centering
  \begin{figureframe}
%      \includegraphics[width=\textwidth]{usr/graphics/static/isorhamnetin_frag}
    \includegraphics[width=\textwidth]{usr/graphics/static/scheme_1-8_isorhamnetin_frag}
 \end{figureframe}
...
The complementary nature of \gls{cid} and \gls{hcd} is especially striking, when comparing spectra of \molref{diosmetin} and \molref{chrysoeriol}.
...
However, with the help of \textit{in silico} methods for the interpretation of \gls{msms} spectra \cite{Bocker2008,Wolf2010} and the computing power available today, the information contained in highly complex spectra might become more easily accessible.
...
In summary, the complementary nature of the studied activation methods \gls{cid} and \gls{hcd} provides more thorough data for the study of flavonoids.
...
%% \begin{itemize}
%% \item non-methylated compounds give better fragmentation (CID)
...
%% \item ONLY methylated flavones and flavonols (kaempferide, isorhamnetin, acacetin, chrysoeriol,diosmetin) show loss of m/z 44.02622 under HCD conditions (more in 100eV energy mode) \textrightarrow subsequent loss of CH4 and CO 
%%   \item \mz{170} (C7H6O5) fragment in HCD spectra of methylated flavonols and flavones and myricetin
%% \item flavones and flavonols (kaempferide, isorhamnetin, myricetin, quercetin, kaempferol,luteolin, apigenin) also show loss of m/z 29.00274 \textrightarrow CHO? formyl radical?
