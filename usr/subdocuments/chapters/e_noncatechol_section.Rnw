...
The previous section showed the conversion of  non-catecholic \ifa by \pfomt and promted additional experiments with other different non-catechols from multiple flavonoid subgroups (\autoref{tab:substitutions}, \autoref{fig:substitutions}).
The tested substrates were selected from four different compound groups (cinnamic acids, flavones, flavanones and flavonols) and each group contained each of four structural motifs (phenol, catechol, \glsentryfirst{3o4m}, \glsentryfirst{4o3m}), if possible.
...
\begin{table}[htb]\centering\small
  ...
  \begin{subfigure}{\textwidth}
    \begin{figureframe}
        \includegraphics[width=0.8\textwidth]{usr/graphics/static/substrate_pattern}
    \end{figureframe}
  \end{subfigure}
...
High pH supported the conversion of non-catechols.

There was almost complete conversion of the catecholic substrates (eriodictyol, luteolin, caffeic acid and quercetin) after 16\,h of incubation regardless the reaction conditions, at least when the wild-type enzyme was used (\autoref{fig:pfomt_conversion} and \autoref{tab:pfomtconversion}).
...
\begin{figure}[htb]
  \begin{figureframe}
    ...
    \end{subfigure}
  \end{figureframe}
  \caption{Conversion of multiple different substrates, catecholic and non catecholic, by PFOMT wild-type (\textbf{\subref{subfig:conv_wt}}) and the 4'-specific variant Y51R N202W (\textbf{\subref{subfig:conv_var}}). Every individual box represents one substrate \textit{p}-coumaric acid (A.1), $\ldots$, kaempferol (D.2).
 ...
 {\footnotesize Codes: \textglobfall (low pH), \textglobrise (high pH), \xmark (no \cf{Mg^2+}), \cmark(yes, \cf{Mg^2+})}}
  \label{fig:pfomt_conversion}
\end{figure}
%
Overall, methylation of \gls{3o4m} motifs was highest.
...
Only some conversion of homoeriodictyol ($\approx$\SI{4}{\percent}), \ifa ($\approx$\SI{3}{\percent}) and kaempferol ($\approx$\SI{7}{\percent}) was observed under high pH/\cf{Mg^2+} conditions (\autoref{tab:pfomtconversion}, \autoref{fig:pfomt_conversion}).
...
The products and methylation sites were confirmed by comparison to authentic standards, or by \gls{lcmsms} (\autoref{tab:conv_products}). 
...
However, enzymatic methylation of the non-catecholic cinnamic acids afforded different products.
...
\begin{table}[htb]\centering\small
  ...
  \begin{tabular}{l>{\centering}p{0.33\textwidth}>{\centering}p{0.33\textwidth}}
   ...
   chrysoeriol & 3',4'-dimethyl luteolin\tup{\textdagger} & 3',4'-dimethyl luteolin\tup{\textdagger}   \tabularnewline
   \midrule
