...
The active site of each monomer was found to be empty except for a sole sulfate ion, which was positioned where the amino- and carboxylate groups of the \gls{sah} residue in the \href{http://www.rcsb.org/pdb/explore/explore.do?structureId=3c3y}{3C3Y} structure (\autoref{fig:d_struct_b}).
...
An overview of the features in the \emph{apo}-\gls{pfomt} structure. \textbf{\subref{fig:d_struct_a}} -- The assymetric unit of \textit{apo}-\gls{pfomt} consists of two homodimers (4 monomers). Individual monomers are rainbow colored from N- (blue) to C-terminus (red). %
 \textbf{\subref{fig:d_struct_b}} -- Comparison of \href{http://www.rcsb.org/pdb/explore/explore.do?structureId=3c3y}{3C3Y} (steelblue) and \textit{apo}-\gls{pfomt} (green). The N-terminus of \textit{apo}-\gls{pfomt} was resolved up to the N-terminus (red) and even the His-tag (red, transparent) was partly resolved.
